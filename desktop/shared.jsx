/* ============================================================
   FitTrack redesign — shared interactive primitives
   Loaded as Babel JSX. Exports to window.
   Style-agnostic: every component takes color/size via props so
   each visual direction can theme it.
   ============================================================ */
const { useState, useEffect, useRef, useMemo } = React;

/* run a callback once on mount after `delay` ms -> boolean "ready" */
...
/* animated number count-up. Re-runs when `value` changes. */
function useCountUp(value, { dur = 1100, decimals = 0, delay = 0 } = {}) {
  ...
  const f = decimals > 0 ? n.toFixed(decimals) : Math.round(n).toLocaleString();
  return f;
}

function CountUp({ value, decimals = 0, dur = 1100, delay = 0, prefix = "", suffix = "" }) {
  const v = useCountUp(value, { dur, decimals, delay });
  return <span>{prefix}{v}{suffix}</span>;
}

/* Catmull-Rom -> cubic bezier smooth path through [{x,y}] in px space */
...
/* map a numeric series into px points within a box */
function toPoints(values, { w, h, pad = 0, min, max }) {
...
/* line that "draws on" via stroke-dashoffset when revealed */
...
/* fade + rise wrapper, staggered by index */
...
/* tiny segmented language toggle — themeable via props */
function LangSwitch({ lang, setLang, bg, fg, active, activeFg, border }) {
  const opts = ["es", "en"];
  return (
    <div style={{
      display: "inline-flex", padding: 3, borderRadius: 999,
      background: bg, border: `1px solid ${border}`, gap: 2,
    }}>
      {opts.map((o) => (
        <button key={o} onClick={() => setLang(o)} style={{
          border: "none", cursor: "pointer", padding: "5px 12px",
          borderRadius: 999, fontSize: 12, fontWeight: 700, letterSpacing: ".04em",
...
Object.assign(window, {
  useReveal, useCountUp, CountUp, smoothPath, toPoints, DrawLine, Rise, LangSwitch,
});
