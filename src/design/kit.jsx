...
import { useReveal, CountUp } from "./shared.jsx";

export const M = {
  paper: "#f3efe6", panel: "#f8f5ee", panel2: "#efeadf",
  ink: "#16140d", ink2: "#6a655a", ink3: "rgba(22,20,13,0)",
  line: "rgba(22,20,13,0.16)", hair: "rgba(22,20,13,0.10)",
  acc: "#e7531c", accDim: "rgba(231,83,28,0.12)",
  ok: "#3f7d4e", okDim: "rgba(63,125,78,0.14)",
  disp: "'Archivo', sans-serif", mono: "'IBM Plex Mono', monospace",
  navH: 78, topPad: 60,
};
...
export function useLocal(key, initial) {
  ...
  useEffect(() => {
    try { localStorage.setItem("ft:" + key, JSON.stringify(v)); } catch {}
  }, [key, v]);
  return [v, setV];
}

export function haptic() { try { if (navigator.vibrate) navigator.vibrate(8); } catch {} }
...
export function Divider({ style }) { return <div style={{ height: 1, background: M.hair, ...style }} />; }

export function BigNum({ value, unit, size = 56, color, count, decimals = 0, delay = 0 }) {
  return (
...
export function PrimaryBtn({ children, onClick, tone = "ink", disabled, style }) {
...
export function Stepper({ value, onChange, step = 1, min = 0, suffix, w = 118 }) {
  const btn = (label, d) => (
...
export function MacroBar({ value, target, color, hero, animate = true }) {
...
export function Ring({ value, max, size = 132, stroke = 13, color, children }) {
...
export function Icon({ name, color = M.ink2, size = 24, fill = false }) {
  const sw = 2;
...
export function TopBar({ kicker, title, right, big = true }) {
  return (
...
export function BottomNav({ tab, setTab, t }) {
  const items = [
...
export function ScreenScroll({ children, refEl }) {
  return (
