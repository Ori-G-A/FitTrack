...
// Ritmos por defecto de cada modo; el deslizador sigue afinando el valor exacto.
const PACE_MODES = [
  ["deficit", "Déficit", -0.4],
  ["mantener", "Mantener", 0],
  ["volumen", "Volumen", 0.25],
];
const modeOf = (weeklyChange) => (Number(weeklyChange) < 0 ? "deficit" : Number(weeklyChange) > 0 ? "volumen" : "mantener");
...
export default function SettingsScreen({ goals, setGoals, weights, exportData, userEmail, signOut, deleteAllData }) {
  ...
  // Lo que importa aqui es si la configuracion alcanza para calcular algo.
  const ready = [referenceWeight ? 1 : 0, Number(goals.targetWeight) ? 1 : 0, goals.autoMacros ? 1 : 0].reduce((sum, value) => sum + value, 0);
  const status = !referenceWeight
    ? { text: "Falta tu peso: registra uno en Cuerpo o escribe el inicial para poder calcular calorías y proteína." }
    : !Number(goals.targetWeight)
      ? { text: "Falta tu peso meta: sin él no se puede estimar cuánto queda ni a qué ritmo vas." }
      : goals.autoMacros
        ? { text: `Todo listo: ${goals.kcalTarget} kcal y ${goals.proteinTarget} g de proteína, recalculados solos con tu peso.` }
        : { text: "Cálculo automático apagado: tus calorías y proteína no se actualizan cuando cambia tu peso." };

  return (
    <>
      <ScreenMast kicker="FITTRACK · AJUSTES" title="Ajustes" />
      <Headline
        kicker="Lo que importa ahora · Configuración"
        sub={`Objetivo ${direction || "sin definir"} · ritmo ${Number(goals.weeklyChange) > 0 ? "+" : ""}${Number(goals.weeklyChange).toFixed(2)} kg/semana`}
        value={ready}
        unit="/3"
        valueSub="ajustes completos"
      >
        {status.text}
      </Headline>
      ...
      <div className="ft-profile">
        ...
        <div>
          <b>{(userEmail || "").split("@")[0] || "Tu cuenta"}</b>
          <small>{latest ? `${latest} kg` : "sin peso registrado"}{Number(goals.targetWeight) ? ` · meta ${goals.targetWeight} kg` : ""}</small>
        </div>
      </div>
      ...
      <div className="ft-cols">
        <div>
          <div className="ft-card">
            ...
            <div className="ft-toggle" style={{ display: "flex", width: "100%", flexWrap: "nowrap" }}>
              {PACE_MODES.map(([key, label, pace]) => (
                <button key={key} className={modeOf(goals.weeklyChange) === key ? "on" : ""} style={{ flex: 1, minWidth: 0 }} onClick={() => updateGoal("weeklyChange", pace)}>{label}</button>
              ))}
            </div>
